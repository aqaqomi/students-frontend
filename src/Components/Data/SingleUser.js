...
const SingleUsers = (props) => {
    ...
    const [item, setItem] = useState(useLocation().pathname.replace("/user/",""));
    ...
    const fetchData = async () => {
        try {
            setItemLoading(true);
            setItemError(false);
            const data = await axios.post(`${Config.API_URL}${Config.ADMIN_API}${Config.VIEW}${apiPath}`, {mobile: item}, headers);
            setItemError(false);
            setItemLoading(false);
            setItem(data.data.data);
            setUpdateInput1(data.data.data.firstName)
            setUpdateInput2(data.data.data.lastName)
            setUpdateInput3(data.data.data.mobile)
        } catch (err) {
            setItemError(true);
            setItemLoading(false);
            setItem(null);
    ...
    const updateItem = async () => {
        ...
        await axios.put(`${Config.API_URL}${Config.ADMIN_API}${Config.UPDATE}${apiPath}`, {mobile: updateInput3, firstName: updateInput1, lastName: updateInput2}, headers)
        .then (response => {
      ...
      const changePassword = async (item) => {
        ...
        if (item && currentPassword && password && passwrdRepeat) {
            setPassLoading(true);
      ...
      const deleteItem = async (item) => {
        ...
        if (item !== "system") {
            await axios.post(`${Config.API_URL}${Config.ADMIN_API}${Config.DELETE}/user`, {mobile: item}, headers)
            .then(result => {
